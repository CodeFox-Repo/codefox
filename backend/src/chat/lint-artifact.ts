 ...
 *
 * Ported from open-design (apps/daemon/src/lint-artifact.ts), Apache
 * License 2.0. Kept verbatim apart from this header so upstream fixes
 * can be diffed straight back in.
 *
...
function extractCssTokens(html: string): CssTokenScope[] {
  ...
  for (const styleBlock of html.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)) {
    ...
    while ((m = ruleRe.exec(css)) !== null) {
      ...
      // Explicit generics: this backend compiles with `noImplicitAny: false`,
      // so `m` is `any` and both collections would otherwise infer as
      // `Set<unknown>` / `Map<any, any>`.
